...
const CardwithTitleBottom = ({ article, onArticleClick }) => {
  const imageUrl = article?.urlToImage || defaultImage;

  return (
    <div className="relative bg-white shadow-md w-full h-full" onClick={() => onArticleClick(article)}>
      {/* Article Image */}
      <img
        src={imageUrl}
        alt={article?.title}
      ...
      <div className="absolute bottom-4 left-4 text-white w-full px-4">
        ...
        <p className="text-white-500 font-medium text-sm py-5">
          <span className='bg-blue-500 px-2'>
            Source: {article?.source?.name}
          </span>
        </p>

        {/* Title */}
        <h3 className="font-semibold text-xs md:text-base lg:text-lg mb-2 bg-black bg-opacity-50 p-2 rounded-md">
          {article?.title}
        </h3>
